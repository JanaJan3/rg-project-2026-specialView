...
vec3 calc_dir_light(vec3 normal, vec3 view_dir, vec3 base_color)
{
    ...
    return (diffuse + specular) * dirLight_color * dirLight_intensity;
}

vec3 calc_point_light(vec3 normal, vec3 view_dir, vec3 base_color)
{
    vec3 light_dir = normalize(pointLight_position - FragPos);
    float diff = max(dot(normal, light_dir), 0.0);
    vec3 halfway = normalize(light_dir + view_dir);
    float spec = pow(max(dot(normal, halfway), 0.0), 32.0);

    float dist = length(pointLight_position - FragPos);
    float attenuation = 1.0 / (pointLight_constant + pointLight_linear * dist +
    pointLight_quadratic * dist * dist);

    vec3 diffuse = diff * base_color;
    vec3 specular = spec * vec3(0.5);
    return (diffuse + specular) * pointLight_color * pointLight_intensity * attenuation;
}
...
void main()
{
    ...
    vec3 dir = calc_dir_light(normal, view_dir, base_color);
    vec3 point = calc_point_light(normal, view_dir, base_color);
    FragColor = vec4(ambient + dir + point, 1.0);
}